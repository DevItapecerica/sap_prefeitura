...
export class GerarEspelhoPontoBolsistaUseCase {
  ...
  async execute(bolsistaId: string, editalId: string, mes: string) {
    ...
    if (
      inicioVinculo > periodo.fim ||
      (fimVinculo && fimVinculo < periodo.inicio)
    ) {
    ...
    const days = this.days(periodo.inicio, periodo.fim).map((data) => {
      ...
      const inLink =
        data >= inicioVinculo && (!fimVinculo || data <= fimVinculo);
      const hasAbsence = faltasPorDia.has(data);
      const situacao = !inLink
        ? "Fora do vínculo"
        : weekday === 0
          ? "DSR"
          : weekday === 6
      ...
      return {
        ...
        situacao,
        horarioPrevisto:
          inLink && weekday > 0 && weekday < 6 ? "07:00 16:00" : "",
        marcacoes: [] as string[],
    ...
    return this.pdfGateway.execute({
      ...
      servidor: {
        matricula: String(person.id || bolsistaId),
        nome: String(person.nome || ""),
        cargo: "Bolsista",
        unidade: String(person.local || ""),
        horario: "07:00 16:00",
        localTrabalho: String(person.local || ""),
        endereco: address,
      },
      periodo: {
        referencia: `${mes.slice(5, 7)}/${mes.slice(0, 4)}`,
  ...
  private parseMonth(value: string) {
    if (!/^\d{4}-(0[1-9]|1[0-2])$/.test(value || ""))
      throw ftError(400, "Mes invalido");
    const [year, month] = value.split("-").map(Number);
    const last = new Date(Date.UTC(year!, month!, 0)).getUTCDate();
    return {
      inicio: `${value}-01`,
      fim: `${value}-${String(last).padStart(2, "0")}`,
    };
  }
