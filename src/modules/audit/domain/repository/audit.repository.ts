import { AuditOutboxItemDto, AuditQueryDto } from "../../application/dto/audit.dto.js";
...
export default interface AuditRepository {
  ...
  claimBatch(now: Date, staleBefore: Date, limit?: number): Promise<AuditOutboxItemDto[]>;
  persist(item: AuditOutboxItemDto): Promise<void>;
  markFailed(item: AuditOutboxItemDto, error: unknown, nextAttemptAt: Date): Promise<void>;
  backlog(): Promise<number>;
  list(query: AuditQueryDto): Promise<{ rows: Record<string, any>[]; count: number }>;
