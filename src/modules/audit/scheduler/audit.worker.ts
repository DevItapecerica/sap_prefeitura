...
export const runAuditWorkerTask = async (
  ...
  try {
    ...
    if (result.backlog > 0)
      logger.warn({ backlog: result.backlog }, "Audit outbox backlog");
    if (result.failed > 0)
      logger.error({ failed: result.failed }, "Audit events failed");
  } catch (error) {
    logger.error({ err: error }, "Audit worker failed");
  } finally {
    running = false;
  }
};

export const runAuditRetentionTask = async (
  logger: FastifyBaseLogger,
  now = new Date(),
) => {
  try {
...
export const startAuditWorker = (logger: FastifyBaseLogger) => {
  const wakeHandler = () => {
    void runAuditWorkerTask(logger);
  };
  ...
  retentionTimer = setInterval(
    () => void runAuditRetentionTask(logger),
