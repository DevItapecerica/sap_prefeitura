  ...
  up: async (queryInterface: QueryInterface): Promise<void> => {
    await queryInterface.sequelize.transaction(async (transaction) => {
      const serviceExists = await queryInterface.rawSelect(
        "services",
        {
          where: { id: CARTERINHA_SERVICE_ID },
          transaction,
        },
        "id",
      );
      ...
      if (!serviceExists) {
        await queryInterface.bulkInsert(
          ...
          [
            {
              id: CARTERINHA_SERVICE_ID,
              name: "Carteirinhas",
              description: "Gerenciamento de carteirinhas",
              url: "/services/10/carterinhas",
              tag: "outros",
              createdAt: new Date(),
              updatedAt: new Date(),
            },
        ...
        const permissionExists = await queryInterface.rawSelect(
          "permissions",
          {
            where: {
              ...
              service_id: CARTERINHA_SERVICE_ID,
            },
            transaction,
          },
          "id",
